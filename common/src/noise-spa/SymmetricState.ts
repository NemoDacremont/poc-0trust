...
export class SymmetricState {
  ...
  private hkdf(ikm: Buffer, salt: Buffer, size: number) {
    return Buffer.from(
      hkdfSync(this.hashName, ikm, salt, Buffer.alloc(0), size),
    );
  }
  ...
  encryptAndHash(plaintext: Buffer): Buffer {
    const ciphertext = this.cs.hasKey()
      ? this.cs.encrypt(this.h, plaintext)
      : plaintext;
  ...
  decryptAndHash(ciphertext: Buffer): Buffer {
    const plaintext = this.cs.hasKey()
      ? this.cs.decrypt(this.h, ciphertext)
      : ciphertext;
    this.mixHash(ciphertext);
